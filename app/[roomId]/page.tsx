    ...
    const fetchRoom = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Show toast for owner name
          if (roomData.ownerName) {
            toast.success(`Room created by ${roomData.ownerName}`);
          }
          ...
          alert("This room has been deleted or has expired.");
          router.push("/");
          return;
        } else {
          const errorData = await response
      ...
      const fetchContent = async () => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            console.log(
              `🚪 Room ${roomId} has been deleted, redirecting to home`
            );
            alert(
              `This room has been deleted by ${room?.ownerName || "someone"}.`
            );
            router.push("/");
            return;
          } else {
            console.log(
              `⚠️ Content polling failed with status: ${response.status}`
            );
          }
        } catch (err) {
