...
export default function ContentCard({ item }: ContentCardProps) {
  ...
  const getCardClasses = () => {
    const baseClasses =
      "bg-white/80 backdrop-blur-sm rounded-xl shadow-lg border border-orange-100 flex flex-col";
    ...
    >
      <div className="p-3 bg-gradient-to-r from-orange-50 to-amber-50 border-b border-orange-100 flex justify-between items-center">
        <div className="flex items-center space-x-2">
          <span
            className={`px-2 py-1 rounded-full text-xs font-medium ${generateUserColor(
              item.userId
            )}`}
          >
            {item.userName}
